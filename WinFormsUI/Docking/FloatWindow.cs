                ...
                case (int)Win32.Msgs.WM_NCLBUTTONDOWN:
                    {
                        ...
                        uint lParam = Win32Helper.MakeLong(ptMouse.X+50, ptMouse.Y);
                        uint result = Win32Helper.IsRunningOnMono ? 0 : NativeMethods.SendMessage(this.Handle, (int)Win32.Msgs.WM_NCHITTEST, 0, lParam);
                        if (result== (uint)Win32.HitTest.HTMINBUTTON)//最小化按钮
                        {
                            base.WndProc(ref m);
                            return;
                        }
